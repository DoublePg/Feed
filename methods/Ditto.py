    ...
    def __init__(self,
        ...
        for i in range(self.N_parties):
            ...
            model_A_twin.fit(private_data[i]["X"], private_data[i]["y"],
                             ...
                             )  # pre-train on local clients

            print("full stack training done")
            global_model = clone_model(model_A_twin)
            global_model.compile(optimizer=tf.keras.optimizers.Adam(lr=1e-3),
                                 loss="sparse_categorical_crossentropy",
                                 metrics=["accuracy"])
    ...
    def collaborative_training(self):
        ...
        while True:
            ...
            for index, d in enumerate(self.collaborative_parties):
                ...
                if collaboration_performance[index][-1] > d['best_global_record']['acc']:  # 比较acc
            ...
            for index, d in enumerate(self.collaborative_parties):
                d["global_model"].set_weights(avg_w)

            print("updates models ...")
            for index, d in enumerate(self.collaborative_parties):
                # 开始正常训练global model
                print("global model {0} starting training with private data... ".format(index))
                d["global_model"].fit(self.private_data[index]["X"],
                                      self.private_data[index]["y"],
                                      batch_size=self.private_training_batchsize,
                                      epochs=self.N_private_training_round,
                                      shuffle=True, verbose=0)

                print("local model {0} starting training with private data... ".format(index))
                dloss = ditto_loss(self.lmd, d["global_model"].trainable_weights)
                d["model_classifier"].compile(optimizer=tf.keras.optimizers.Adam(lr=1e-3),
                                              loss=lambda y_true, y_pred: dloss(y_true, y_pred, d[
