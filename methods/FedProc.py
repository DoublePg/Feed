...
from losses import fedproc_loss
...
class FedProc():
    def __init__(self,
        ...
        for i in range(self.N_parties):
            ...
            model_A_twin.compile(optimizer=tf.keras.optimizers.Adam(lr=1e-3, decay=1e-3),
    ...
    def collaborative_training(self):
        ...
        while True:
            ...
            for index, d in enumerate(self.collaborative_parties):
                ...
                d["model_classifier"].set_weights(weights_to_use)  # 将经知识蒸馏后的模型参数复制给原模型

                # 更新alpha
                d['alpha'] = 1 - r / self.N_rounds

                zs = []
                for label in self.private_data[index]["y"]:
                    if self.dataset == 'yelp':
                        zs.append(global_protos[label])
                    else:
                        zs.append(global_protos[label])
                zs = np.array(zs)

                ploss = fedproc_loss(global_protos)
                d["model_classifier"].compile(loss={'pred': "sparse_categorical_crossentropy",
                                                    'embeddings': lambda y_true, y_pred: ploss(y_true, y_pred)},
                                              loss_weights={
                                                  'pred': 1.0 - d["alpha"],
                                                  'embeddings': d["alpha"]
                                              },
